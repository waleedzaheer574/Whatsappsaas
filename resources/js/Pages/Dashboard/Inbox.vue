...
onMounted(() => {
  ...
  if (window.Echo && props.workspace) {
    window.Echo.channel(`workspace.${props.workspace.id}`)
      .listen('MessageReceived', (e) => {
        if (activeChat.value && activeChat.value.id === e.message.conversation_id) {
          messageRows.value.push(e.message);
          nextTick(() => {
        ...
        // Update sidebar chat rows
        const chatIdx = chatRows.value.findIndex(c => c.id === e.message.conversation_id);
        if (chatIdx !== -1) {
          chatRows.value[chatIdx].last_message_at = e.message.created_at;
          if (!activeChat.value || activeChat.value.id !== e.message.conversation_id) {
            chatRows.value[chatIdx].unread_count = (chatRows.value[chatIdx].unread_count || 0) + 1;
          }
        }
      });
  }
