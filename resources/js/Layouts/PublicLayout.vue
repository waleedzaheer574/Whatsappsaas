<template>
  <div class="min-h-screen bg-[radial-gradient(circle_at_top_left,rgba(124,58,237,.16),transparent_35%),linear-gradient(180deg,#fff,#f8fafc)] dark:bg-[radial-gradient(circle_at_top_left,rgba(124,58,237,.22),transparent_35%),linear-gradient(180deg,#070B1A,#0F172A)]">
    <header class="fixed inset-x-0 top-0 z-50 border-b border-white/70 bg-white/82 shadow-sm backdrop-blur-2xl dark:border-white/10 dark:bg-ink/82">
      <nav class="section-wrap flex h-20 items-center justify-between">
        <AppLogo />
        ...
        <button class="grid size-11 place-items-center rounded-2xl border border-slate-200 bg-white dark:border-white/10 dark:bg-white/10 lg:hidden" @click="open = !open">
          <Menu class="size-5" />
        </button>
      </nav>
    ...
    <footer class="border-t border-slate-200 bg-white/70 py-12 dark:border-white/10 dark:bg-white/[.03]">
      <div class="section-wrap grid gap-10 md:grid-cols-[1.2fr_repeat(4,1fr)]">
        <div>
          <AppLogo />
